...
public static void main(String[] args)
{
	...
	System.err.println("Simulator: loading configuration " + argv[0]);
	...
	try {                
		for(int k=0; k<exps; ++k)
		{
			...
			System.err.print("Simulator: starting experiment "+k+ " at time "+data.toString());
			System.err.println(" invoking "+simName[SIMID]);
			System.err.println("Random seed: "+
				CommonState.r.getLastSeed());
                        if(k==0){
				CommonState.setArguments(argv);
				argv=null;                                
                        }
                        System.err.println("\n\n"+"Config "+CommonState.getConfig()+"\n\n");
                        newout = (PrintStream)Configuration.getInstance(PAR_REDIRECT,System.out);
			...
			case EDSIM:
				...
				break;
                        ...
                        if(newout != System.out)
                            newout.close();
                        data = new Date(new GregorianCalendar().getTime().getTime() - data.getTime());
                        System.err.println("Simulator: finishes experiment "+k+ " in "+data.getTime()+" ms\n");
		}
